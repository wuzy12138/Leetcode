...
public:
    ...
    vector<vector<int>> threeSum(vector<int>& nums, int l, int target) {
      ...
      // int r = n - 1;
      for (int i = l; i < n; ++i){
        vector<vector<int>> twoSumResult = twoSum(nums, i+1, target - nums[i]);
    ...
    vector<vector<int>> nSum(vector<int>&nums, int target, int sumN){
      vector<vector<int>> result;
      sort(nums.begin(), nums.end());
      int n = nums.size();

      if (sumN < 2 || n < 2) return result;
      if (sumN == 2) {
        result = twoSum(nums, 0, target);
        return result;
      }
      else{
        for (int i = 0; i < n; ++i){
          vector<vector<int>> nSumResult = nSum(nums, i+1, target - nums[i]);
          // printVector(threeSumResult);
          if (nSumResult.size() != 0) {
            for(vector<int> temp : nSumResult){
              temp.push_back(nums[i]);
              result.push_back(temp);
            }
          }
...
int main(){
    ...
    int target = 0;
    vector<vector<int>> result = s->nSum(nums, target, 4);
    for(auto i : result){
      for(auto j : i){
        cout << j << " ";
      }
